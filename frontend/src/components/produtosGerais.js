...
const ProdutosGerais = ({ onAddToCart, onModulesLoaded }) => {
  ...
  useEffect(() => {
    ...
    fetch('https://api.devops-platform.40.82.176.176.nip.io/api/modules')
      .then(res => {
        if (!res.ok) {
    ...
    <div className="product-grid">
      {modules.map((p) => (
        <ProductCard
          ...
          onShowDetails={() => {
            const urls = {
              'mod-core': 'https://app.core-engine.40.82.176.176.nip.io/',
              'mod-desk': 'https://app.service-desk.40.82.176.176.nip.io/',
              'mod-crm': 'https://app.crm-leads.40.82.176.176.nip.io/',
              'mod-fiscal': 'https://app.fiscal-finance.40.82.176.176.nip.io/'
            };
            const redirectUrl = urls[p.id];
            if (redirectUrl) {
